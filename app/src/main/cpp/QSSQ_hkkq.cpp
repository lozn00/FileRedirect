#include<jni.h>
#include <android/Foundation/Helper.h>
...
nativeIORedirect(JNIEnv *env, jclass jclazz, jstring origPath, jstring newPath) {
    ScopeUtfString orig_path(env, origPath);
    ScopeUtfString new_path(env, newPath);
...
nativeReverseRedirectedPath(JNIEnv *env, jclass jclazz, jstring redirectedPath) {
    ScopeUtfString redirected_path(env, redirectedPath);
